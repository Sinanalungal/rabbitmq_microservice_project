...
def consumerfunction():
    ...
    while True:
        try:
            ...
            channel.exchange_declare(exchange='coupon_exchange', exchange_type='fanout')
            
            channel.queue_declare(queue='backup_coupon', durable=True)
            channel.queue_bind(exchange='coupon_exchange', queue='backup_coupon', routing_key='backup')
            channel.basic_consume(queue='backup_coupon', on_message_callback=callback, auto_ack=True)
